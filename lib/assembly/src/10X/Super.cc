     ...
     const vec<int>& llens, vec<double>& COV )
{
     ...
     if ( dlines.empty( ) )
          Martian::exit("The assembly graph does not contain any structure that could represent genomic regions longer than 10 kb and Supernova will not continue. This is a signal of a serious problem with the input data.");
     ...
     digraphE<vec<int>>& D, vec<int>& dinv, ReadPathVec& dpaths )
{
     ...
     KillMisassembledCells( hb, dup, bci, paths, D, dinv, dpaths, dpaths_index,
          bc, dlines, dels, fhist, BC_REQUIRE, BC_FLANK, BC_IGNORE, False);
     ...
     const int BC_REQUIRE, int BC_FLANK, int BC_IGNORE, const Bool verbose )
{
     ...
     // by going out of scope
     {
          ...
          FindMoleculesOnLines( dlines, llens, lrpb, fhist, mol_lwml,
     ...
     const digraphE<vec<int>>& D, const vec<int>& dinv )
{
     ...
     {    int e = bads[bi];
          if ( dinv[e] < 0 )
          {    cout << "\nInvolution not fully defined." << endl;
               TracebackThisProcess( );
               Scram(1);    }
          if ( dinv[dinv[e]] != e )
     ...
     digraphE<vec<int>>& D, vec<int>& dinv )
{
     ...
     cout << Date( ) << ": found " << ToStringAddCommas(joins) << " joins" << endl;
     D.DeleteEdges(dels);
     Zipper( D, dinv );
     return joins;    }

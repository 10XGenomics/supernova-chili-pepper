...
)
{
     ...
     {    const vec<int>& Y = ( ps == 0 ? X : RX );
          ...
          {    ostringstream xout;
               ...
               M.clear( );
               int delta = -1, offset = -1, bandwidth = -1;
               for ( int pass = 1; pass <= 2; pass++ )
               {    M.clear( );
                    offsets.clear( );
                    delta = Abs( G.isize( ) - B.isize( ) );
                    offset = -delta/2, bandwidth = delta + 1;
...
void RefAlign(
     ...
     // control:
     const int verbosity, const vec<int>& test_edges )
{
     // Set up.
     ...
     {    RefAlignScratch<K> scr;
          ...
          {    int d = ds[s];
               ...
               RefAlignCore<K>( D.O(d), HBK, tigs, inv, D, dinv, dlines, genome, 
                    alignsb, galigns[d], verbosity, reports[s], scr );

...
void DefinePairSet( const HyperBasevectorX& hb, const vec<int>& inv, 
     VP& paths, VPI& paths_index, const vec<Bool>& dup, const vec<Bool>& bad,
     const vec<int64_t>& pids,
     vec< pair <vec<int>, vec<int> > >& proc, vec<int64_t>& ppids )
{
     const int64_t batch = 10000;
     #pragma omp parallel for schedule( dynamic, 1 )
     for ( int64_t bi = 0; bi < pids.jsize( ); bi += batch )
     {    vec< pair <vec<int>, vec<int> > > proci;
          vec<int64_t> ppidsi;
          vec<int> x1, x2;
          for ( int64_t i = bi; i < Min( bi + batch, pids.jsize( ) ); i++ )
          {    int64_t pid = pids[i];
               Bool flip = ( pid < 0 );
               if (flip) pid = -pid;
               if ( dup[pid] || bad[pid] ) continue;
               int64_t id1 = 2*pid, id2 = 2*pid+1;
               if (flip) swap( id1, id2 );
...
template< class VP, class VPI > void ClosePair( const vec<int>& x1, 
     const vec<int>& x2, const int64_t pid, const HyperBasevectorX& hb, 
     const vec<int>& inv, VP& paths, VPI& paths_index, int64_t& opcounti, 
     ...
     int64_t& opcount, const Bool GLOBAL, vec<int64_t>& pids, const int verbosity )
{
     ...
     vec<int64_t> ppids;
     DefinePairSet( hb, inv, paths, paths_index, dup, bad, pids, proc, ppids );
     ...
     int64_t batch = ( proc.jsize( ) + batches - 1 ) / batches;
     int ndots = 0;
     #pragma omp parallel for schedule( dynamic, 1 )
     for ( int64_t bi = 0; bi < batches; bi++ )
     {    vec<vec<int>> closures, closuresi;
          int64_t opcountj = 0;
          int64_t start = bi*batch, stop = Min( (bi+1)*batch, proc.jsize( ) );
          ...
          {    ostringstream out;
               ...
               int64_t opcounti = 0;
               vec<int> x1 = proc[pi].first, x2 = proc[pi].second;
               // out << "x: " << printSeq(x1) << " .. " << printSeq(x2) << endl;
...
template void Closer( const HyperBasevectorX& hb, 
     const vec<int>& inv, MasterVec<ReadPath>& paths, 
     MasterVec<ULongVec>& paths_index, const vec<Bool>& dup, 
     const vec<Bool>& bad, vec<vec<int>>& all_closures,
     int64_t& opcount, const Bool GLOBAL, vec<int64_t>& pids, const int verbosity );

template void Closer( const HyperBasevectorX& hb, 
     const vec<int>& inv, VirtualMasterVec<ReadPath>& paths, 

     ...
     )
{
     ...
     if (VERBOSE)
     {    Ofstream( out, "/mnt/home/jaffe/repos/BroadCRD/woof.dot" );
          ...
          {    int gap = off - EE[0].isize( );
               ...
               {    String title = "e1=" + ToString(e1) + ",e2=" + ToString(e2)
                    ...
                    {    gout << "\n" << title << endl;
                         s.Print(gout);    }    }    }    }
     if (VERBOSE)
     {    {    Ofstream( fout, "fido.fasta" );
               fout << out.str( );    }
          SystemSucceed( "QueryLookupTable K=12 MM=12 MC=0.15 SEQS=fido.fasta "
               "L=/mnt/home/jaffe/refs/hg19/genome.lookup VISUAL=True NH=True "
               "QUIET=True MF=5K SMITH_WAT=True QUERY_NAMING=from_record > snork" 
               );    
          Remove( "fido.fasta" );
          fast_ifstream in( "snork" );
          String line;
          while(1)
          {    getline( in, line );
               if ( in.fail( ) ) break;
     ...
     if (VERBOSE)
     {    Ofstream( out, "/mnt/home/jaffe/repos/BroadCRD/woof1.dot" );
     ...
     {    hbl2.ToLeft(left2), hbl2.ToRight(right2);
          ...
          if (VERBOSE) gout << ", below, and printing as woof2.dot";
          gout << endl;
          if (VERBOSE)
          {    Ofstream( out, "/mnt/home/jaffe/repos/BroadCRD/woof2.dot" );
               hbl2.PrintSummaryDOT0w( out, False, False, True );    }
          // NOTE EXPENSIVE CONVERSION!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
          GetSupport( hb, HyperBasevectorX(hbl2), basesy, support );
          if (VERBOSE)
               ...
               {    {    Ofstream( out, "bpaths.fasta" );
                    ...
                    {    gout << "\nALIGNMENT OF PATHS FOR ";
                         ...
                         SystemSucceed( "QueryLookupTable K=12 MM=12 MC=0.15 "
                              "SEQS=bpaths.fasta "
                              "L=/mnt/home/jaffe/refs/hg19/genome.lookup "
                              "VISUAL=True NH=True QUIET=True MF=5K SMITH_WAT=True "
                              "QUERY_NAMING=from_record > woofyx" );
                         CpAppend( "woofyx", gout );
                         if ( paths.isize( ) <= MAX_CLOSURES )
                         {    fast_ifstream in( "woofyx" );
                              while(1)
                              {    String line;
                                   getline( in, line );
                                   if ( in.fail( ) ) break;

...
//     vec<Bool> dup1( X.size( ), False );
     ...
     cout << setiosflags(ios::fixed) << setprecision(2)
          << 100.0 * Sum(dup) / ( bases.size( )/2 ) << resetiosflags(ios::fixed)
     ...
     cout << setiosflags(ios::fixed) << setprecision(2)
          << 100.0 * Sum(art) / ( bases.size( )/2 ) << resetiosflags(ios::fixed)
...
void AllTinksCore( const HyperBasevectorX& hb, const vec<int>& inv,
     const ReadPathVec& paths, const vec<int64_t>& bci, const vec<vec<int>>& ebc,
...
{    cout << Date( ) << ": start AllTinks" << endl;
     ...
     int NBC = bci.size( ) - 1;
     int64_t npaths = paths.size( );
     cout << Date( ) << ": have " << ToStringAddCommas(npaths) << " reads" << endl;
     int64_t unbar = bci[1];
     ...
     {    int e1 = qep[i].first ,e2 = qep[i].second;
          ...
          qept[i].third = Intersection( ebc[e1], ebc[e2] ).size( );     }
     ...
     const Bool verbose )
{
     ...
     vec<int64_t> pids = vec<int64_t>( paths.size( )/2, vec<int64_t>::IDENTITY );
     int64_t opcount = 0;
     Closer( hb, inv, paths, paths_index, dup, bad, all_closures, opcount,
          True, pids, ( verbose ? 1 : 0 ) );

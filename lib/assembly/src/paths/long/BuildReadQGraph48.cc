...
public:
    Kmerizer( vecbvec const& reads, std::vector<unsigned> const& goodLengths,
                ...
                Dict* pDict, std::atomic_size_t* pTotKmers )
...
public:
    ...
    HBVPather( vecbvec const& reads, VirtualMasterVec<PQVec> quals,
...
void pathReads( vecbvec const& reads, VirtualMasterVec<PQVec>& quals,
                Dict const& dict, vecbvec const& edges,
                HyperBasevector const& hbv,
                vec<int> const& fwdEdgeXlat, vec<int> const& revEdgeXlat,
                ...
                Bool const VERBOSE = False )
{
    ...
    HBVPather pather(reads,quals.clone(),dict,edges,hbv,fwdEdgeXlat,revEdgeXlat,
...
void buildReadQGraph48(String const& work_dir,
                       ...
                       vecbvec const& reads, ObjectManager<VecPQVec>& quals,
                       ...
                       bool const VERBOSE )
{
  ...
  if (mspFilename.size() > 0) {
    ...
    pDict = createDict(reads,quals,minQual,minFreq,ignBcBelow,memFrac,minBC,bcp);
...
//  if ( doJoinOverlaps ) joinOverlaps(reads,K/2,minFreq2,&edges,pDict);
  ...
  if ( !pPaths )
  ...
  else
    {
      ...
      pathReads(reads,vquals,*pDict,edges,*pHBV,
